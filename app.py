...
def run_optimization(all_items):
    ...
    if best_solution:
        ...
        for idx, t in enumerate(best_solution):
            t.name = f"{t.name} (No.{idx+1})" # 이름 임시 저장
        ...
        if 'run_result' in st.session_state:
            ...
            if not items: st.error("데이터 변환 실패.")
                ...
                if trucks:
                    ...
                    # [New UI] Split Layout with Interactive Table
                    c_list, c_detail = st.columns([2, 3])
                    
                    with c_list:
                        st.markdown("### 🚛 배차 리스트 (클릭하여 상세보기)")
                        summary_data = []
                        for idx, t in enumerate(trucks):
                            ...
                            summary_data.append({
                                "No.": idx + 1,
                                "차량": t.name.split(' (')[0], 
                                "체적": vol_pct, # Column name simplified
                                "중량": weight_pct, # Added Weight Ratio
                                "비용": t.cost
                            })
                        ...
                        event = st.dataframe(
                            ...
                            column_config={
                                ...
                                "체적": st.column_config.ProgressColumn(
                                    format="%.0f%%", min_value=0, max_value=1, width="small"
                                ),
                                "중량": st.column_config.ProgressColumn(
                                    format="%.0f%%", min_value=0, max_value=1, width="small"
                                ),
                                "비용": st.column_config.NumberColumn(format="%d", width="small")
                            }
                        )
                    ...
                    with c_detail:
                        ...
                        display_name = target_truck.name.split(' (')[0]
                        st.markdown(f"### 🔍 상세 뷰: [{selected_idx+1}] {display_name}")
                        ...
                        # [수정] 동일한 디자인의 막대 그래프 2개 배치
                        c_gauge1, c_gauge2 = st.columns(2)
                        with c_gauge1:
                            st.markdown(f"**📏 체적 적재율 ({vol_pct*100:.1f}%)**")
                            st.progress(vol_pct)
                        with c_gauge2:
                            st.markdown(f"**⚖️ 중량 적재율 ({weight_pct*100:.1f}%)**")
                            st.progress(weight_pct)
