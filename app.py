...
# 2. 설정 및 데이터
# ==========================================
st.set_page_config(layout="wide", page_title="Ultimate Load Planner (Final_v3)")
...
def draw_truck_3d(truck, camera_view="iso"):
    ...
    def create_wheel(cx, cy):
        # 타이어 (완전 무광, 밝은 회색으로 강제 설정하여 어두움 방지)
        ...
        # [핵심] ambient=1.0 (그림자 0%), diffuse=0.0 (난반사 0%) -> 빛의 영향 0%
        ...
        tx, ty, tz = [], [], []
        for i in range(16):
            t1, t2 = (2*np.pi/16)*i, (2*np.pi/16)*(i+0.5)
    ...
    # 3. 적재함 벽면 (대각선 삭제를 위해 투명도 극대화 + 테두리만 강조)
    # 벽면은 거의 투명하게 처리해서 대각선이 안 보이게 함
    wall_c = [[0, 'rgba(240,240,240,0.1)'], [1, 'rgba(240,240,240,0.1)']] # 투명도 0.1
    # 좌/우/앞/뒤
    ...
    dim_line([-offset,L,0], [-offset,L,1300], f"높이제한(최대4단): 1300", c='red')
...
# 5. 메인 UI
# ==========================================
st.title("📦 Ultimate Load Planner (Final_v3)")
st.caption("✅ 대각선 삭제 | 바퀴 그림자 제거 | 2축 바퀴 | 디자인 완성")
...
if uploaded_file:
    ...
    if 'run_result' in st.session_state:
        ...
        if not items: st.error("데이터 변환 실패.")
            ...
            if trucks:
                st.success(f"✅ 분석 완료: 총 {len(trucks)}대")
