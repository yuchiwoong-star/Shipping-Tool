...
class Truck:
    ...
    def put_item(self, item):
        ...
        # 피벗 정렬: Z(바닥) -> Y(안쪽) -> X(왼쪽) 
        # 안전 우선: 왼쪽 벽면부터 채우기 위해 X 오름차순
        ...
        for p in self.pivots:
            ...
            if pz > 0.001:
                ...
                # 적재 단수 계산
                # 바로 아래에 있는 박스의 level 중 최대값 + 1
                max_below_level = 0
                item_cx, item_cy = px + item.w/2, py + item.d/2
                
                # 지지해주는 박스 찾기 (최적화: Z축 근접 박스만)
    ...
    def _check_support_fast(self, item, x, y, z):
        # 지지력 검사 최적화: 바로 아래(Z값 근접) 박스만 검사
        support_area = 0.0
        item_area = item.w * item.d
        required = item_area * 0.8
        
        for exist in self.items:
            # 바로 아래에 있는 박스인가? (오차범위 1mm)
...
def run_optimization(all_items):
    ...
    # [수정] 정렬 로직 개선: 폭(Width) 우선 -> 길이(Depth) -> 높이(Height)
    # 폭이 넓은 박스를 먼저 배치하여 '단단한 벽'을 형성, 빈 공간(Gaps) 최소화
    def get_optimized_sorted_items(items_to_sort):
        # 1순위: 폭(w) 내림차순 (벽면 채우기 유리)
        # 2순위: 길이(d) 내림차순 (같은 폭이면 긴 것 먼저)
        # 3순위: 무게(weight) 내림차순 (무거운 것 먼저)
        return sorted(items_to_sort, key=lambda x: (x.w, x.d, x.weight), reverse=True)

    def solve_remaining_greedy(current_items):
        used_trucks = []
        rem = current_items[:]
        
        # 미리 정렬 (매 반복마다 정렬하지 않음)
        rem = get_optimized_sorted_items(rem)
        ...
        while rem:
            ...
            for t_name in TRUCK_DB:
                ...
                if count > 0:
                    # 효율성 점수: (적재중량 / 비용) + (적재개수 가산점)
                    eff = (w_sum / spec['cost']) 
                    
                    # 우선순위: 1. 남은거 다 실을 수 있는가? 2. 비용 효율이 좋은가?
                    is_all = (count == len(rem))
                    
                    if is_all:
                        # 다 실린다면 비용이 가장 싼게 장땡
                        current_cost_score = 1.0 / spec['cost']
                        if best_truck is None or (current_cost_score > max_eff):
                            max_eff = current_cost_score
                            best_truck = t
                    else:
                        # 다 못 실으면 가성비 따짐
                        if eff > max_eff:
    ...
    # [최적화] 모든 차종을 'Start Truck'으로 시도하되, 정렬은 한 번만
    sorted_all_items = get_optimized_sorted_items(all_items)
